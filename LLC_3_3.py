...
class Battery:
    ...
    def __init__(self, stages_num, Oin, Ain, yin, xin, Kclass, eff=1., convergence=1E-4):
        ...
        if np.isscalar(eff):
            eff_list = np.ones(stages_num) * eff
        else:
            if len(eff) != stages_num:
                print ('Length of efficiency array is not correct.')
            eff_list = np.ones(stages_num)* eff

        # k=0
        for i in range(stages_num):
            self.stages[i] = Stage(self.Oin_list[i], self.Ain_list[i], self.yin_list[i], self.xin_list[i], Kclass=self.Kclass, eff=eff_list[i])
        ...
        for i in range(stages_num):
            self.stages[i] = Stage(self.Oin_list[i], self.Ain_list[i], self.yin_list[i], self.xin_list[i], Kclass=Kclass, eff=eff_list[i])
            self.Oout_list[i] = self.stages[i].Oout
            self.Aout_list[i] = self.stages[i].Aout
            self.yout_list[i] = self.stages[i].yout
            self.xout_list[i] = self.stages[i].xout          
