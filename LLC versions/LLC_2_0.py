...
class Stage:
    ...
    def __init__(self, Oin, Ain, yin, xin, Kclass, convergence=1E-3):
        self.Oin = float(Oin)
        self.Ain = float(Ain)
...
class Battery:
    """Attributes:
	  ...
	  """
    def __init__(self, stages_num, Oin, Ain, yin, xin, Kclass, convergence=1E-3):
        ...
        # self.yin_list = np.ones(stages_num)*yin
        # self.xin_list = np.ones(stages_num)*xin
        self.Kclass = Kclass
        ...
        # self.yout_list = np.zeros(stages_num)
        # self.xout_list = np.zeros(stages_num)

        # k=0
        for i in range(stages_num):
            self.stages[i] = LLC.Stage(self.Oin_list[i], self.Ain_list[i], self.yin_list[i], self.xin_list[i], Kclass=self.Kclass)
        ...
        for i in range(stages_num):
            self.stages[i] = LLC.Stage(self.Oin_list[i], self.Ain_list[i], self.yin_list[i], self.xin_list[i], Kclass=Kclass)
            self.Oout_list[i] = self.stages[i].Oout
            self.Aout_list[i] = self.stages[i].Aout
            self.yout_list[i] = self.stages[i].yout
            self.xout_list[i] = self.stages[i].xout          
        ...
        self.xin_list = np.array(self.xin_list)
        self.xout_list = np.array(self.xout_list)
        self.yin_list = np.array(self.yin_list)
        self.yout_list = np.array(self.yout_list)
